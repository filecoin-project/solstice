...
Net roughly +47B on the 165B current f02 state root block.
...
streams[]        [ id, WeightRecord[v_start, slope, t_start, floor, cap], distribution ]
  ...
  EXPLICIT  = [ writer,       # FIP "designated writer": may call SetShares.
      ...
      claimed_period[]: [[wallet, amount], ...]  # claims this period; subtract Σ from
                                                 ...
                                                 # (see supply accounting)
      claimed_total ]                            # lifetime; survives removal

tombstones[]     [ [id, claimed_total, payable[]], ... ]
                 # removed streams. claimed_total is kept forever (permanent term in
                 # derived S and M; see supply). payable[] carries the removed
                 # stream's outstanding liabilities so claims stay addressable after
                 # removal; rows delete as they're claimed.
pending_writes[] [ id, op, payload, effective_epoch, origin(mechanism|discretionary) ]
                 ...
                 # later; the hold is a floor).
                 # SetShares never queues. origin exists only for CancelPending:
                 # mechanism (gate) writes can't be cancelled, discretionary
                 # (governance) can; the tag is caller-asserted (f02 doesn't know).
```
...
|---|---|---|
| `AwardBlockReward` (per block) | read (weight records); write only when applying a due queued write | `total_minted`, `total_burn`, `service_accrued`; `next_transition` on apply |
| `SetWeightRecords` / `RegisterStream` / `RemoveStream` / `SetDistribution` | write: queue entry (second write later, at application) | `next_transition_epoch` |
...
| `Claim` | write: claimed rows, `payable`, `claimed_total` | none |
...
```
size ≈ 100 + S·(70 + 20·(2N + P))   # S streams, N recipients, P payable rows; queue/tombstones extra, same row arithmetic
S=1, N=16: ~1.1 KB (v1: fine)    S=8, N=32: ~16 KB (heavy)    S=100, N=1000: ~6 MB (not ok!)
```

`MAX_STREAMS` and `MAX_RECIPIENTS` are named but never valued in the FIP. We should set both low (single digits and low tens) and have the FIP assert that raising them requires a future FIP and network upgrade that also reshapes the affected structures for the new scale (using appropriate expandable data structures). Tombstones likely don't need a cap: growth is gated on `RemoveStream`, a governance-cadence op, and a drained tombstone is ~15B.

We avoid storing floating point values directly on chain, instead using various mechanisms to derive fractional values. The fractions in this FIP (the weight fields and shares) are represented as `u64` fixed point, `DENOM = 1e18`. `1e18` over a power of two because the FIP percentages are exact integers in it, and `Σ shares == DENOM` becomes an exact integer check; the `w * BR` multiply promotes to bignum then `div_floor`s. Slope quantisation over the 9-quarter ramp is ~1e-10pp at this scale and the clamp endpoints are stored exact values, so the ramp rests at exactly 50% regardless *(nicer alternative would be to store the ramp by endpoints instead of a slope)*.
...
f02's own supply accounting becomes `T = M + B + S`.

* `T` is position 9.
* `B` (`total_burn_minted`) is newly stored because w0's f099 contribution is otherwise difficult to recover independently
* `S` derives from claim state: `S = Σ over streams of (service_accrued - Σ claimed_period + Σ payable + claimed_total)`, tombstones included
* `M`, what miners have received, is the derived residual `T - B - S`; it is not stored.
...
Due writes apply in effective-epoch order (id as tiebreak) at the start of the award and of every mutating method, so no message ever executes against stale config; a later write to the same target simply overwrites the earlier. A small queue cap keeps the apply step trivially bounded.
