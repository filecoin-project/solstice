...
The live addresses are in [`deployments.json`](../deployments.json) (`sra`, `swa`, and the current `sraImplementation` and `swaImplementation`) and are hardwired into Lotus by [lotus#13809](https://github.com/filecoin-project/lotus/pull/13809). The verification record for that deployment, with explorer links, is in [#84](https://github.com/filecoin-project/solstice/pull/84).
